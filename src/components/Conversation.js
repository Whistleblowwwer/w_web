    ...
    <div>
      <div className='conversation-container'>
        <div className='flex justify-center fixed w-[100%] top-0 left-[19%]'>
          <p
            className='text-zinc-900 flex items-center justify-center text-center p-2 text-xl font-medium leading-[18.62px] h-12 w-[66%] translate-x-[6%]'
            style={{ backgroundColor: 'rgba(169, 169, 169, 0.06)' }}
          >
        ...
        <div className='messages mt-6'>
          {props.messages && props.messages.length > 0 ? (
            props.messages.map((message, index) => (
              <div key={index} className={`message ${message._id_sender === props.userId ? 'sent' : 'received'}`}>
                {message.content}
                {message.createdAt && <span className='ml-2 text-[12px] opacity-70'>{format(new Date(message.createdAt), 'hh:mm a')}</span>}
              </div>
            ))
          ) : (
      ...
      <div className='conversation-response'>
        <div className='fixed w-[100%] top-[9%] z-10000'>
          <span className='absolute mt-2 ml-2' onClick={props.handleChatVisible}>
            <i className='fa-solid fa-arrow-left'></i>
          </span>
          <p
            className='flex justify-center items-center text-zinc-900 text-center p-2 text-xl font-medium leading-[18.62px] h-12 w-[100%] '
            style={{ backgroundColor: 'rgba(169, 169, 169, 0.06)' }}
          >
            <img src={props?.profile_picture_url || proSet} alt='Perfil' className='w-10 h-10 rounded-full mr-2' />
