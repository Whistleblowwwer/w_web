...
const Home = ({ setAuth }) => {
    ...
    const handleSearch = (e) => {
        ...
        async function getSearchs() {
            ...
            try {
                const response = await fetch(
                    `http://18.220.124.246:4000/users/search?searchTerm=${e}`,
                    requestOptions
                );
                const parseRes = await response.json();
    ...
    const handleAddPost = () => {
        if (textPost && selectedImages.length > 0) {
            ...
            async function postReview() {
                ...
                const response = await fetch(
                    "http://18.220.124.246:4000/reviews",
                    requestOptions
                );
    ...
    const handleSearchCompanyClick = () => {
        async function getCompanies() {
            ...
            try {
                const response = await fetch(
                    "http://18.220.124.246:4000/business/search?",
                    requestOptions
                );
                const parseRes = await response.json();
            ...
            */}
                        ...
                        <div className="w-[66%] h-auto post-post">
                            {postes.map((post, index) => (
                                <div
                                    key={index}
                                    className={`bg-[#FFF] h-auto w-[100%] p-3 mt-1 ${
                                        darkMode ? "dark-register-bg" : ""
                                    }`}
                                >
                                    <button
                                        ...
                                        onClick={() =>
                                            handleBusinessClick(post.Business)
                                        }
                                    >
