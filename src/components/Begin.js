...
function Begin() {
    ...
    return (
        <div className={`contain-begin overflow-y-hidden flex justify-center items-center h-screen w-screen ${darkMode ? 'dark-register-bg' : ''}`}>
            ...
            <div className="contain-principal-begin flex ml-[12%]">
                ...
                <div className="w-1/2 p-4 flex flex-col contain-principal-begin">
                    <h1 className={`w-[580px] text-neutral-900 text-[70px] font-bold leading-[88px] mb-8 ${darkMode ? 'dark-text-white' : ''}`}>Lorem ipsum <br /> dolor sit amet</h1>
                    ...
                    <button className={`hover:bg-gray-700 py-2 px-4 mt-4 w-[280px] h-[41px] bg-neutral-900 rounded-[44px] relative ${darkMode ? 'dark-button' : ''}`}>
                        ...
                        />
                        <span className={`text-stone-50 text-[15px] font-medium leading-[13.96px] pl-6 ${darkMode ? 'dark-text' : ''}`}>Registrase con Google</span>
                    </button>
                    <button 
                        className={`hover:bg-gray-700 py-2 px-4 mt-4 w-[280px] h-[41px] bg-neutral-900 rounded-[44px] relative ${darkMode ? 'dark-button' : ''}`}
                        onClick={() => handleButtonClick('/register')} 
                    >
                        <span className={`text-stone-50 text-[15px] font-medium leading-[13.96px] ${darkMode ? 'dark-text' : ''}`}>Crear cuenta</span>
                    </button>
                    <p className={`text-neutral-900 text-[12px] font-medium mt-4 ${darkMode ? 'dark-text-white' : ''}`}>Al registrarte, aceptas los <span className="underline">Términos de servicio</span> y la <span className="underline">Política
                    <br /> de privacidad,</span> incluida la política de <span className="underline">Uso de Cookies.</span></p>
                    <br />
                    <p className={`text-neutral-900 text-lg font-medium leading-none ${darkMode ? 'dark-text-white' : ''}`}>¿Ya tienes una cuenta?</p>
                    <button 
                        className={`hover:bg-gray-200 py-2 px-4 mt-4 w-[280px] h-[41px] border border-neutral-900 rounded-[44px] relative" ${darkMode ? 'dark-button-login' : ''}`}
                        onClick={() => handleButtonClick('/login')}
                    >
                        <span className={`text-neutral-900 text-[15px] font-medium leading-[13.96px] ${darkMode ? 'dark-text-white' : ''}`}>Iniciar sesión</span>
                    </button>
                </div>
            </div>
        </div>
