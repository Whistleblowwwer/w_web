...
function SearchCurrentChat(props) {
  ...
  useEffect(() => {
    setFilteredSuggestions(
      props?.suggestions.filter((suggestion) => suggestion?.name.toLowerCase().includes(props.usersSearchQuery.toLowerCase()))
    )
        ...
        <ul className='absolute z-50 w-full h-full mt-1 bg-white border rounded-md shadow-md'>
          {filteredSuggestions.map((suggestion) => (
            <li
              ...
              className={classNames('p-2 cursor-pointer bg-[#FFF] hover:bg-gray-100', {
